...
{
	if (key == GLFW_KEY_ESCAPE) TheApp::running = false;
...
static void ChangeMousePos(int x, int y)
{
...
void main()
{
...
#ifdef FULLSCREEN
	window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "Renderer Editor", glfwGetPrimaryMonitor(), 0 );
#else
#ifdef DOUBLESIZE
	window = glfwCreateWindow( SCRWIDTH * 2, SCRHEIGHT * 2, "Renderer Editor", 0, 0 );
#else
	#if GAMETYPE == GAME
		window = glfwCreateWindow( SCRWIDTH * UPSCALE, SCRHEIGHT * UPSCALE, "Renderer Editor", 0, 0 );
	#else
		window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "Renderer Editor", 0, 0);
	#endif
#endif
#endif
	...
	app = Renderer::getInstance();//new Renderer();
	app->screen = screen;
	app->Init();
	...
	//io.Fonts->AddFontFromFileTTF("../assets/fonts/DS-DIGI.TTF", 16.0f);
	//io.Fonts->Build();
	...
	while (!glfwWindowShouldClose( window ))
	{
		...
		if (frameNr++ > 1)
		{
			...
			// Draw Bullet Debugger Here
			if (Renderer::getInstance()->COLLIDERS)
			{
				Renderer::getInstance()->debugger->setShader(&Renderer::getInstance()->camera);
				Renderer::getInstance()->dynamicsWorld->debugDrawWorld();
			}
...
#if GAMETYPE == GAME
				glViewport(0, 0, display_w * UPSCALE, display_h * UPSCALE);
#else
